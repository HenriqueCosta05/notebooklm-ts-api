...
The app mounts `src/`, `locales/`, and `.env` as read-only volumes and runs `ts-node-dev` for live reloading.

### Production build

```bash
docker build -t notebooklm-ts-api .
docker run -p 3000:3000 \
  -e NOTEBOOKLM_AUTH_JSON="$(cat storage_state.json)" \
  notebooklm-ts-api
```
...
3. **`runner`** — minimal Alpine image with compiled JS and a non-root user
